...
public interface ChunkTrackingView {
    ...
    default boolean contains(final ChunkPos pos) {
        return this.contains(pos.x().intValueExact(), pos.z().intValueExact());
    }
    ...
    record Positioned(ChunkPos center, int viewDistance) implements ChunkTrackingView {
        private int minX() {
            return this.center.x().intValueExact() - this.viewDistance - 1;
        }

        private int minZ() {
            return this.center.z().intValueExact() - this.viewDistance - 1;
        }
        ...
        @Override
        public boolean contains(final int chunkX, final int chunkZ, final boolean includeNeighbors) {
            return ChunkTrackingView.isWithinDistance(this.center.x().intValueExact(), this.center.z().intValueExact(), this.viewDistance, chunkX, chunkZ, includeNeighbors);
        }
