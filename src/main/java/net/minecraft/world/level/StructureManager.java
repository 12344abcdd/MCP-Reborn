...
public class StructureManager {
    ...
    public List<StructureStart> startsForStructure(final ChunkPos pos, final Predicate<Structure> matcher) {
        Map<Structure, ObjectSet<ChunkPos>> allReferences = this.level.getChunk(pos.x().intValueExact(), pos.z().intValueExact(), ChunkStatus.STRUCTURE_REFERENCES).getAllReferences();
        Builder<StructureStart> result = ImmutableList.builder();

        for (Entry<Structure, ObjectSet<ChunkPos>> entry : allReferences.entrySet()) {
    ...
    public List<StructureStart> startsForStructure(final SectionPos pos, final Structure structure) {
        ObjectSet<ChunkPos> referencesForStructure = this.level.getChunk(pos.x(), pos.z(), ChunkStatus.STRUCTURE_REFERENCES).getReferencesForStructure(structure);
        Builder<StructureStart> result = ImmutableList.builder();
        this.fillStartsForStructure(structure, referencesForStructure, result::add);
        return result.build();
    }

    public void fillStartsForStructure(final Structure structure, final ObjectSet<ChunkPos> referencesForStructure, final Consumer<StructureStart> consumer) {
        for (ChunkPos key : referencesForStructure) {
            SectionPos sectionPos = SectionPos.of(key, this.level.getMinSectionY());
    ...
    public Map<Structure, ObjectSet<ChunkPos>> getAllStructuresAt(final BlockPos pos) {
        SectionPos sectionPos = SectionPos.of(pos);
